...
function matchScore(queryText: string, article: ArticleWithType, extraKeywords?: string[]): number {
  ...
  // Exact phrase match in title (stripped of "Roll to Defend")
  const strippedQuery = normalizedQuery.replace(/roll to defend\s*/g, '').trim()
  const strippedTitle = normalizedTitle.replace(/roll to defend\s*/g, '').trim()
